...
class SimMPET(dae.daeSimulation):
    ...
    def SetUpVariables(self):
        ...
        #GG2
        self.m.h_h.AssignValue(config["h_h"])
        self.m.Omega_a_c.AssignValue(config["c","Omega_a"][0,0])
        if not config["prevDir"] or config["prevDir"] == "false":
            #GG2  
            #if config['profileType'] == 'BBcontrol':
            self.m.Tavg.SetInitialCondition(config["T0"])
            # Solids
            for tr in config["trodes"]:
                cs0 = config['cs0'][tr]
            ...
            #GG2
            self.m.current.SetInitialGuess(utils.get_dict_key(data, "current", final=True))
            self.m.power.SetInitialGuess(utils.get_dict_key(data, "power", final=True))
            for tr in config["trodes"]:
            ...
            #GG2
            #if config['profileType'] == 'BBcontrol':
            self.m.Tavg.SetInitialCondition(utils.get_dict_key(data, "Tavg", final=True))
            ...
            self.m.t0.AssignValue(utils.get_dict_key(data, "phi_applied_times", final = True))
    ...
    def SetUpSensitivityAnalysis(self): #GG2
        ...
        self.SetSensitivityParameter(self.m.h_h)
        self.SetSensitivityParameter(self.m.Omega_a_c)
