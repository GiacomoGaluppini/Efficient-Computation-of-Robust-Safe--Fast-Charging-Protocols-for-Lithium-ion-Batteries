...
class MyMATDataReporter(daeMatlabMATFileDataReporter):
    ...
    def WriteDataToFile(self):
        ...
        for var in self.Process.Variables:
            ...
            #GG2                
            #print(dkeybase)
            if 'sensitivities' in dkeybase:
                dkeybase=dkeybase.replace("sensitivities", "SS")
                dkeybase=dkeybase.replace("(", "")
                dkeybase=dkeybase.replace(")", "")
                #print(dkeybase)
        ...
        sio.savemat(self.ConnectionString + ".mat",
                    ...
                    long_field_names=True, do_compression=False,
                    oned_as='row')
...
def setup_data_reporters(simulation, config, outdir):
    ...
    if not config["prevDir"] or config["prevDir"] == "false":
        ...
        matfilename = os.path.join(config["prevDir"], matDataName)
