...
class muRfuncs():
    ...
    def graphite_1param_homog_A123(self, y, Omga, Omgb, ISfuncs=None):
        """ Helper function with low hysteresis and soft tail """
        width = 5e-2
        tailScl = 5e-2
        muLtail = -tailScl*1./(y**(0.85))
        muRtail = 1.0e1*step_up(y, 1.0, 0.015)
        muLlin = (0.15*Omga*12*(0.40-y**0.98)
                  * step_down(y, 0.7, 0.9*width)*step_up(y, 0.5, width))
        muRlin = (0.1*Omga*4*(0.74-y) + 0.90*Omgb - 2*(1-y))*step_up(y, 0.4, 0.4*width)
        muLMod = (0.
                  + (40*(-np.exp(-y/0.025))- 2*(1-y))*step_down(y, 0.05, width)
                  + 1*(np.tanh((y-0.37)/0.075) - 1)
                  + 1*(np.tanh((y-0.2)/0.06) - 1)
                  + 0.1*(np.tanh((y-0.16)/0.015) - 1)
                  )*step_down(y, 0.5, width)
        muR = 0.18 + muLMod + muLtail + muRtail + muLlin + muRlin - 0.2
        return muR
    ...
    def LiFePO4(self, y, ybar, T, muR_ref, ISfuncs=None):
        ...
        muRhomog = self.reg_sln(y, T, self.m.Omega_a_c, ISfuncs)
    ...
    def LiC6_1param_A123(self, y, ybar, muR_ref, ISfuncs=None):
        muRtheta = -self.eokT*0.12
        ndD = self.ndD
        muRhomog = self.graphite_1param_homog_A123(
